    ...
    <div class="form-container">
      ...
      <form action="submit_offer.php" method="POST">
        <input type="hidden" name="loan_id" value="<?php echo !empty($_GET['loan_id']) ? htmlspecialchars($_GET['loan_id']) : 0; ?>" />
          ...
          <input type="number" id="interest" name="interest_rate" placeholder="Percentage of interest" required>
          ...
          <p>Loan ID passed in hidden field: <?php echo !empty($_GET['loan_id']) ? htmlspecialchars($_GET['loan_id']) : 'No loan_id'; ?></p>
      </form>
  </div>
  </div>
</body>
